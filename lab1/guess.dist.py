...
def recvmsg(prompt):
    sendmsg(prompt.encode())  # UTF-8 轉 bytes
    msg = input().strip()
    msg = base64.b64decode(msg.encode()) # base64 解碼
    mlen = int.from_bytes(msg[0:4], 'little')  # 用 Litte-Endian 解讀前四 bytes
    if len(msg)-4 != mlen: sendmsg(msg4.encode()); sys.exit(1)  # 檢查前四個 bytes 的整數是否等於 msg 長度
    m = zlib.decompress(msg[4:]).decode()  # 解壓
    return m
